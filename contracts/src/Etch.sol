...
/// @notice Permanent onchain records on Abstract. Typed, optionally soulbound ERC-721 tokens
///         with generative art embedded in token metadata.
...
contract Etch is ERC721Enumerable, Ownable, Pausable {
    // -- Types ----------------------------------------------------------------
    ...
    /// @param uri       Metadata URI (data:application/json;base64,... with embedded SVG art).
    ...
    /// @notice Update the metadata URI for an existing token. Owner or minter only.
    ...
    /// @notice Burn a token. Only the token owner can burn their own token.
    ///         Soulbound tokens can be burned (destroyed) but not transferred.
    /// @param tokenId  Token to burn.
    function burn(uint256 tokenId) external {
        if (ownerOf(tokenId) != msg.sender) {
            revert NotTokenOwner(tokenId);
        }
        // Temporarily clear soulbound flag so _update allows the burn
        _tokenData[tokenId].soulbound = false;
        _burn(tokenId);
